...
//
// Uninstalls new Module.
//
// Uninstalls a hoodie module
//
// Options:
//
...
CreateCommand.prototype.execute = function(options, callback) {
  ...
  self.exec(which('npm'), ['remove', options.module, '--save'], function(err) {

    if (err) {
      self.hoodie.emit('warn', 'Error uninstalling module:');
      process.exit(1);
      throw err;
    }

    self.hoodie.emit('log', 'Successfully uninstalled ', options.module);
