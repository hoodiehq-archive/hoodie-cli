...
CreateCommand.prototype.execute = function(options) {
  ...
  if (options.link) {
    ...
    shell.exec(shell.which('npm') + ' link', { silent: true }, function(err) {
    ...
    async.map(pluginsArr, function (p, cb) {
      ...
      self.hoodie.emit('info', 'Installing ' + plugin + ' plugin(s)');

      shell.exec(shell.which('npm') + ' ' + npmArgs.join(' '), { silent: false }, function(err) {
