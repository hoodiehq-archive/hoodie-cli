...
CreateCommand.prototype.run = function(options, callback) {
  ...
  options.name = options.template.repo || 'my-first-hoodie';
...
CreateCommand.prototype.fetch = function (options, ctx, callback) {
  ...
  console.log(self.getCacheDir() + options.template.repo);

  // skip checkout if we already have a copy
  //
  if (!fs.existsSync(self.getCacheDir() + options.template.repo)) {

    if (options.template.branch) {
      gitArgs.push('-b', options.template.branch);
    }
...
CreateCommand.prototype.cleanup = function(options, ctx, callback) {
  ...
  var self = ctx;
  var rmArg = self.getCacheDir() + options.template.repo + '/.git';
...
CreateCommand.prototype.rename = function (options, ctx, callback) {
  ...
  process.chdir(self.getCacheDir() + options.template.repo);
...
Command.prototype.copyToCwd = function (options, ctx, callback) {
  ...
  var self = ctx;
  var srcDir = self.getCacheDir() + options.template.repo + '/';
