...
CreateCommand.prototype.run = function(options, callback) {
  ...
  options.cacheDir = options.npmInstall ? npmUtils.getNPMCacheDir() : dirUtils.getCacheDir();
...
CreateCommand.prototype.fetch = function (options, ctx, callback) {
  ...
  if (!options.npmFetch) {
    ...
    shell.exec(npmUtils.bin() + ' install -g ' + options.defaultTmpl + ' ' + options.npmArgs , function(err) {
      if (err) {
        self.hoodie.emit('error', 'Could not fetch node module');
        return callback(err);
      }
...
CreateCommand.prototype.execute = function (options) {
  ...
  async.applyEachSeries(runOrder, options, self, function (err) {
