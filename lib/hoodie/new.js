...
var npmbin = require('./util/npm-bin');
var dirutil = require('./util/dir');
...
CreateCommand.prototype.run = function(options, callback) {
  ...
  options.template.uri = dirutil.buildGitURI(options);
  options.plugins = options.plugins || [];
  options.cwd = process.cwd();
  options.cacheDir = dirutil.getCacheDir();
...
CreateCommand.prototype.deps = function (options, ctx, callback) {
  ...
  shell.exec(npmbin.bin() + ' ' + options.npmArgs.join(' '), options.execArgs, function(err) {
    if (err) {
