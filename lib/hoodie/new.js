...
CreateCommand.prototype.fetch = function (options, ctx, callback) {
  ...
  if (!fs.existsSync(options.cacheDir + options.template.repo)) {
    ...
    shell.exec(shell.which('git') + ' ' + options.gitArgs.join(' '), { silent: true }, function(err) {
...
CreateCommand.prototype.deps = function (options, ctx, callback) {
  ...
  var self = ctx;
  var npmArgs = ['install', '--production', '--cache-min 99999999'];
  var execArgs = {
    async: true,
    silent: true
  };
  ...
  if (options.verbose) {
    delete execArgs.silent;
    execArgs.silent = false;
    npmArgs.push('--loglevel', 'silly');
  }
  ...
  shell.exec(shell.which('npm') + ' ' + npmArgs.join(' '), execArgs, function(err) {
    if (err) {
...
Command.prototype.copyToCwd = function (options, ctx, callback) {
  ...
  if (shell.exec('mkdir -p ' + path.dirname(targetDir), { silent: true }).code !== 1) {

    self.hoodie.emit('info', 'preparing ' + options.name + '...');
    shell.exec('cp -R ' + srcDir + ' ' + targetDir, { async: true }, function (err) {
